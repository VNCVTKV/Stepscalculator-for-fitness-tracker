...
def write_data(today_date: str, calories_burned: float,
...
def time_difference_in_hours(time1: Union[str, dt.time],
                             time2: Union[str, dt.time]) -> float:
    """
...
def check_correct_time(time: Union[str, dt.time], storage_data: dict) -> bool:
    ...
    if storage_data:
        max_time = max(map(lambda x: dt.datetime.strptime(x, FORMAT),
                       storage_data.keys())).time()
...
def get_spent_calories(storage_data: dict, dist: float,
                       current_time: Union[str, dt.time]) -> float:
    """
    ...
    previous_time = dt.datetime.strptime(list(storage_data.keys())[-1],
                                         FORMAT).time()
    distance_time = time_difference_in_hours(current_time, previous_time)
    mean_speed = dist / distance_time
    return (WEIGHT * (K_1 + K_2 * (((mean_speed) ** 2) / HEIGHT))
            * distance_time * 60)
...
def get_actual_message(calories_burned: float, storage_data: dict) -> str:
    ...
    if distance >= 6.5:
        ...
        message = ("Lying down is also useful. "
...
def accept_package(data: tuple[str, int]) -> None:
    ...
    if (check_correct_data(data) and
       (check_correct_time(data[0], storage_data) or len(storage_data) == 1)):
        calories_burned += get_spent_calories(storage_data,
                                              get_distance(data[1]), data[0])
